...
public class UserController{
	...
	/**
	 * Connects to the database, used to store passwords (All necessary tables, in fact -
	 * simple table Users must be already created).
	 * @param stmnt Statement to use for query execution (from the existing database connection).
	...
	/**
	 * Retrieves user's information by it's Unique ID, except for session ID and Activation code.
	...
	public static User getUserByID(Integer iD){
		...
		return result;}
	/**
	 * Function checks, is entered fields data is correct, and if so, registers new user.
	 ...
	 * @param needUserConfirmation If true, there will be generated activation code,
	 * and user will be marked as non-confirmed.
	 ...
	 * @return Object of class User, if login was successful, and false in other case.
	 */
	...
	public boolean login(String Username,String Password,String SessionID){
		...
		return result;}
	/**
	 * Closes session for specified user, if it have opened session (doesn't matter, if it's up-to-date or not).
	 * @param user User, whose session must be closed.
	 * @return True, if session was closed, or false, if error was occurred during attempt.
	 */
	...
	public boolean changePassword(User user,String oldPassword,String newPassword,String newPasswordConfirm){
		...
		if(newPassword.equals(newPasswordConfirm)){
			Session sess = HibernateUtil.openSession();
			user = (User) sess.get(User.class,user.getId());
			if(user.getPass().equals(getMD5(oldPassword))){
	 ...
	 * @return Object of type User, which represents current logged in user(If exists).
	 */
